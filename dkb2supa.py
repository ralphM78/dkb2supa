...
'''
    ...
    Dieses Programm wird in der Hoffnung bereitgestellt,
    dass es nützlich sein wird, jedoch OHNE JEDE GEWÄHR,; sogar ohne die
    implizite Gewähr der MARKTFÄHIGKEIT oder EIGNUNG FÜR EINEN BESTIMMTEN ZWECK.
...
parser.add_argument(
    "-o",
    "--output",
    help="SUPA formated CSV output file",
    type=argparse.FileType(
        "w",
        encoding="utf-8"),
    required=True,
...
parser.add_argument(
    "--iban",
    help="Optionally your IBAN",
    metavar="DE820000000000")
parser.add_argument(
    "--cur",
    default="EUR",
    help="Optional Currency. Default is EUR",
    metavar="EUR")
...
# don't process first 7 lines
    ...
    with args.output as CsvOutputfile:
        ...
        for row in inputData:
            ...
            if match(r"^\[('(\d{2}\.){2}\d{4}', ){2}", str(row)):
                # convert booking date
                BookDt = datetime.strptime(
                    row[0], "%d.%m.%Y").strftime("%Y-%m-%d")
                # convert value date
                ValDt = datetime.strptime(
                    row[1], "%d.%m.%Y").strftime("%Y-%m-%d")
                # remove HTML markup
                br = row[4].replace("<br />", " ")
